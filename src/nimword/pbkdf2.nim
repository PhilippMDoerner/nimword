...
func hashEncode(hash: string, salt: string, iterations: SomeInteger, algorithm: string): string =
...
proc hashEncodeSHA256Pbkdf2*(password: string, salt: string, iterations: int): string {.gcsafe.} =
  ## Hashes the given password with an HMAC using the SHA256 hashing function 
  ## and the PBKDF2 function to derive a key as a "hash" from the password.
  ## This is using openSSL.
  ...
  let hash = hashSHA256Pbkdf2(password, salt, iterations)
  result = hash.hashEncode(salt, iterations, "pbkdf2_sha256")

proc hashEncodeSHA512Pbkdf2*(password: string, salt: string, iterations: int): string {.gcsafe.} =
  ## Hashes the given password with an HMAC using the SHA512 hashing function 
  ## and the PBKDF2 function to derive a key as a "hash" from the password.
  ## This is using openSSL.
  ## The hash is returned in a string together with the algorithm, salt and 
  ## number of iterations used to generate it, following this pattern:
  ## "<algorithm>$<iterations>$<salt>$<hash>" 
  let hash = hashSHA512Pbkdf2(password, salt, iterations)
  result = hash.hashEncode(salt, iterations, "pbkdf2_sha512")
